...
function showCell(evt) {
  ...
  if (evt.target.classList.contains('mine')) {
    //Change the smiley face to a frowning face 
    //document.getElementById('reset').src="https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/240/microsoft/209/white-frowning-face_2639.png";
    displayMessage('BOOM!')
    //Stop the timer
    timer = document.getElementById('timer').innerText

    var snd = new Audio("images/herecomestheboom.mp3"); // buffers automatically when created
    snd.play();
    revealMines()
    removeListeners()
    return
  ...
  console.log(evt.target +" This cell is counting"+cell.surroundingMines )
...
function setInnerHTML(cell) {
  ...
  if (element.classList.contains('hidden')) {
    ...
    // Update font color depending on the number of mines
